...
=======
    ...
    Text, Date, SmallInteger, ForeignKey
>>>>>>> origin/develop
)
...
<<<<<<< HEAD
    ...
    dmnd_instt_nm = Column(String(200))
    bid_mtd_nm = Column(String(100))
    cntrct_cnclsn_mtd_nm = Column(String(100))
    ...
    bid_clse_dt = Column(TIMESTAMPTZ)
    ntce_bgn_dt = Column(TIMESTAMPTZ)
    ntce_end_dt = Column(TIMESTAMPTZ)
    openg_dt = Column(TIMESTAMPTZ)
    exec_term_start_dt = Column(Date)
    exec_term_end_dt = Column(Date)
    ntce_url = Column(Text)
    ...
    collected_at = Column(TIMESTAMPTZ)
    updated_at = Column(TIMESTAMPTZ)

    analysis_result = relationship(
        "AnalysisResult", back_populates="notice", uselist=False
    )
...
<<<<<<< HEAD
    ...
    submit_deadline = Column(TIMESTAMPTZ)
...
<<<<<<< HEAD
    id = Column(Integer, primary_key=True)
    outline_id = Column(
        Integer, ForeignKey("proposal_outlines.id", ondelete="CASCADE"), nullable=False
    )
...
EOFcat > backend/db/models.py << 'EOF'
from sqlalchemy import (
    Column, Integer, String, Boolean, Numeric,
    Text, Date, SmallInteger, ForeignKey
)
