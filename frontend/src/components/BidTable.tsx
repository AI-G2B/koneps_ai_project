...
import { Eye, ExternalLink, Loader2, CheckCircle2, ChevronUp, ChevronDown, ChevronsUpDown, Calendar } from 'lucide-react';
...
export function RiskBadge({ risk }: { risk: RiskLevel }) {
  ...
  return (
    <span
      ...
      style={{ padding: '2px 8px', fontSize: '11px', fontWeight: 500, backgroundColor: c.bg, color: c.text, border: `1px solid ${c.border}` }}
    >
      {c.label}
    </span>
  );
...
export function BidTable({ selectedBid, onSelectBid }: BidTableProps) {
  ...
  const filteredBids = bids.filter((bid) => {
    ...
    if (fromDate) {
      const collectedDate = new Date(bid.collectedAt);
      if (collectedDate < fromDate) return false;
    }
    // 상태 필터
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ padding: '12px 16px', borderBottom: '1px solid var(--dash-border)' }}
      >
        {/* 1행: 타이틀 + 상태 필터 */}
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            ...
            <span
              ...
              style={{ fontSize: '11px', padding: '1px 8px', backgroundColor: 'rgba(37,99,235,0.15)', color: '#2563EB' }}
            >
              {sortedBids.length}건
            </span>
          </div>
              ...
              <button
                ...
                style={{
                  ...
                  backgroundColor:
                    statusFilter === f.key
                      ? f.key === 'all' ? '#2563EB'
                      : f.key === 'urgent' ? 'rgba(239,68,68,0.15)'
                  ...
                  border: `1px solid ${
                    statusFilter === f.key
                      ? f.key === 'all' ? 'transparent'
                      : f.key === 'urgent' ? 'rgba(239,68,68,0.3)'
                      : 'rgba(245,158,11,0.3)'
                      : 'var(--dash-border-btn)'
                  }`,
                }}
      ...
      <div className="flex-1 overflow-auto">
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '680px' }}>
          ...
          <tbody>
            {sortedBids.length === 0 ? (
              <tr>
                <td colSpan={8} style={{ padding: '40px', textAlign: 'center', color: 'var(--dash-text-4)', fontSize: '13px' }}>
                  해당 기간에 수집된 공고가 없습니다
                </td>
              </tr>
            ) : (
      ...
      <div
        ...
        style={{ padding: '8px 16px', borderTop: '1px solid var(--dash-border)' }}
      >
...
function BidRow({ bid, isSelected, urgent, daysLeft, onSelect }: {
    ...
    >
      ...
      <td style={{ padding: '10px 12px', maxWidth: '200px' }}>
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          <span className="rounded" style={{ fontSize: '10px', padding: '0 4px', backgroundColor: 'rgba(37,99,235,0.12)', color: '#60A5FA' }}>
            {bid.type}
          </span>
      ...
      <td style={{ padding: '10px 12px' }}>
        <span style={{ fontSize: '12px', color: 'var(--dash-text-2)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', display: 'block', maxWidth: '100px' }}>
          {bid.agency}
        </span>
      </td>
      <td style={{ padding: '10px 12px' }}>
        ...
        <div>
          ...
          {urgent && (
            <div className="flex items-center gap-1 mt-0.5">
              <span className="rounded-full" style={{ fontSize: '10px', padding: '0 5px', backgroundColor: 'rgba(239,68,68,0.15)', color: '#EF4444' }}>
                D-{daysLeft}
              </span>
            </div>
          )}
      ...
      <td style={{ padding: '10px 12px' }}>
        <div className="flex items-center gap-1">
          <button
            ...
            style={{ width: '28px', height: '28px', color: 'var(--dash-text-3)', backgroundColor: 'transparent' }}
